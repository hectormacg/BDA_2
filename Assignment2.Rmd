...
#priors
#-------------------------------------------------------------------------------
mean.area=mean(eggs$area)
b=max(eggs$num.eggs)/mean.area
a=1/(b*mean.area)
m0=(log(a)+log(b))/2
sigma0=(log(b)-log(a))/2
sigma1=(log(b)-log(a))/2
#-------------------------------------------------------------------------------
#Data
#-------------------------------------------------------------------------------
data_meggs=list(N=N, K=K, y=y, X=X, area_log=area_log, m0=m0, sigma0=sigma0, sigma1=sigma1);
```
...
#effectiveSize
#---------------------------------------------------------
effectiveSize(mcmc_samples[, c('beta[1]', 'beta[2]', 'beta[3]', 'beta[4]')])

#---------------------------------------------------------
#gelman plots and metrics
#---------------------------------------------------------
gelman.diag(mcmc_samples[, c('beta[1]', 'beta[2]', 'beta[3]', 'beta[4]')])
gelman.plot(mcmc_samples[, c('beta[1]', 'beta[2]', 'beta[3]', 'beta[4]')])
...
**Explanation**: The model is formulated as follows:

$$
\mu_i = \text{area}_i \cdot e^{\beta_1 + \beta_{2} \cdot \text{Spread} + \beta_{3} \cdot \text{Lead} + \beta_{4} \cdot \text{Spread\_and\_Lead}}
$$

Given that the response is expected to be between the interval $[0, 340]$, a non-informative prior is chosen such that $\mu_i$ resides within $\left[\frac{1}{340}, 340\right]$, thereby avoiding issues associated with zero. An important consideration is that, due to the area offset, these ranges are adjusted by dividing by the mean area (or mean offset), resulting in a modified range of $\left[\frac{1}{340 \cdot \text{mean\_area}}, \frac{340}{\text{mean\_area}}\right]$. This follows from the equation above by dividing through by $\text{area}_i$. For this range $[a, b]$, the prior formulation is:

$$
\beta_1 \sim N(\mu_0, \sigma^2_0)
$$

where $\mu_0 = \frac{\log(a) + \log(b)}{2}$ and $\sigma_0 = \frac{\log(b) - \log(a)}{2}$.

For the subsequent coefficients $\beta_i$ ($i > 1$), we use:

$$
\beta_i \sim N(0, \sigma^2_i)
$$

where $\sigma_i = \frac{\log(b) - \log(a)}{2}$.

These priors ensure that the response adheres to the desired range.

After running 4 chains each with 3000 iterations, analysis of the Gelman plot indicates that the chains become more stable after 2000 iterations. Additionally, the Gelman-Rubin statistic $\hat{R} < 1.05$ suggests that the chains have reached a stable distribution.
...
**Explanation**: 
Intercept ($\beta_1$): Represents the expected number of eggs in the absence of spraying (sprayed = "N") and lead exposure (lead = "N").

Effect of Spraying ($\beta_2$): Indicates the change in the number of eggs transitioning from "N" to "Y" for the sprayed variable, while keeping lead exposure constant at "N".

Effect of Lead Exposure ($\beta_3$): Denotes the change in the number of eggs transitioning from "N" to "Y" for lead exposure, while keeping the spraying condition constant at "N".

Interaction Effect of Spraying and Lead ($\beta_4$): Represents the added change in the number of eggs when both spraying and lead exposure are applied simultaneously ("Y"). This term quantifies whether the combined effect of these two treatments differs from the sum of their individual impacts.

With regard to the distributions, it can be inferred that spraying is more effective since the corresponding $\beta$ is more negative, indicating a larger decrease in the egg count compared to using only the lead insecticide. Additionally, it can be concluded that applying both treatments rather than just spraying results in a further reduction of egg counts by approximately 10%, as the rate $e^{\beta_3+\beta_4}$ suggests a multiplicative effect around 0.90.
...
**Explanation**: In the negative binomial model, the variance is given by:
...
- **Large $\phi$:**
  - As $\phi$ becomes large, $\frac{\mu^2}{\phi}$ approaches zero, making $\sigma^2 \approx \mu$. This is similar to a Poisson distribution, where variance equals the mean, indicating little to no over-dispersion.

- **Small $\phi$:**
  - A small $\phi$ increases $\frac{\mu^2}{\phi}$, leading to $\sigma^2 > \mu$, showing over-dispersion.

Thus, larger $\phi$ suggests that the data has variance close to the mean, like a Poisson distribution.

using the prior of gamma(2, 0.01) give a mean of 20 which is a high value in this context.
...
**Explanation**: The negative binomial model provides a better balance between goodness-of-fit and
complexity, making it the preferred choice, having a lower WAIC than the poisson. 
...
```{r}
mcmc_dens(res.meggs2, pars=c("theta[1]", "theta[2]", "theta[3]", "theta[4]",
...
**Explanation**: 
For the farm-specific random effects hyper parameters, the hyper-mean parameter $\mu$ was assigned a Normal(0, 1) prior, assuming the average farm-level effect centers around zero on the log scale. The hyper-standard deviation $\tau$ was given an gamma(0.1, 0.1) prior, favoring smaller values and encouraging shrinkage towards the overall mean while allowing larger variation if needed. These priors balance flexibility with regularization, capturing true variation without allowing for implausibly large effects.

The posterior predictive checks indicate that all the KPIs fall within the range of the samples, demonstrating that the posterior predictive function is performing well and appropriately capturing the variability in the data.

Additionally, the posterior predictive analysis for each $\theta_j$ reveals that for farms 5, 9, and 13, the expected count of eggs is lower, as the factor is less than 1. Since none of these distributions are centered around 1 and they display distinct characteristics, it makes sense to employ random effects for each farm.
...
**Explanation**: $$\mu_{\theta} \sim N(0,1) \quad  \tau_{\theta} \sim Gamma(0.1, 0.1) $$
$$\beta \sim N(0, \sigma_1)  \quad \theta \sim N(\mu_{\theta}, \tau_{\theta}) \quad \phi \sim Gamma(0.1, 0.1)$$
$$\eta_{i,j}=log\_area_i+ \theta_j + \beta_{1} \cdot \text{Spread}_i + \beta_{2} \cdot \text{Lead}_i + \beta_{3} \cdot \text{Spread_and_Lead}_i \quad 1<=j<=13 $$
$$\mu_{i,j}=e^{\eta_{i, j}} $$
$$y_{i,j} \sim NegBin(\mu_{i,j}, \quad \phi)$$
...
```{r}
X_new <- rbind(c(0, 0, 0), c(1, 0, 0), c(0, 1, 0),c(1,1,1))
theta_farm_b6<-extract(res.meggs2)$theta[,6]
beta<-extract(res.meggs2)$beta
phi<-extract(res.meggs2)$phi
eta <- t(X_new %*% t(beta)) + theta_farm_b6 + log(30)
y_pred <- apply(eta, 2, \(x) rnbinom(length(x), size = phi, mu = exp(x)))
apply(y_pred, 2, \(x) mean(x==0))
...
# ---------------------------------------------------------------------------------
# Function to plot spatial correlation for a given setting
run_and_plot_spatial_correlation <- function(range_index, sigma_index, edge_index) {
  # Locate the settings using indices
  range_setting <- prior_ranges[[range_index]]
  sigma_setting <- prior_sigmas[[sigma_index]]
  edge_setting <- max_edges[[edge_index]]

  # Create mesh for locations with specified edge setting
  Locations <- data.frame(easting = ozone$Longitude, northing = ozone$Latitude)
  loc.mesh <- inla.mesh.2d(Locations, max.edge = edge_setting)
  
  # SPDE model setup with current priors
  loc.spde <- inla.spde2.pcmatern(mesh = loc.mesh,
                                 prior.range = range_setting,
                                 prior.sigma = sigma_setting)
  
  ozone$sfLocations <- sf::st_as_sf(Locations, coords = c("easting", "northing"))$geometry

  # Define the model
  cmp <- o3 ~ floc(sfLocations, model = loc.spde) + 
                fyear(year_centered, model = year.spde) +
                year_centered + Longitude + Latitude + Intercept(1)

  # Fit the model
  res.ozone <- bru(cmp, data = ozone, family = "gaussian",
                   options = list(control.compute = list(dic = TRUE, cpo = TRUE, waic = TRUE)))

  # Extract posterior correlation
  post.corr <- spde.posterior(res.ozone, name = "floc", what = "matern.correlation")

  # Construct plot data frame
  df_to_plot <- data.frame(
    distance = post.corr$x,
    mean_corr = post.corr$q0.5,
    lower_corr = post.corr$q0.025,
    upper_corr = post.corr$q0.975
  )

  # Create the plot object
  p <- ggplot(df_to_plot, aes(x = distance, y = mean_corr)) +
    geom_line(color = "blue") +
    geom_ribbon(aes(ymin = lower_corr, ymax = upper_corr), alpha = 0.2) +
    labs(
      title = paste("Spatial Correlation with Priors: Range", paste(range_setting, collapse = "-"),
                    ", Sigma", paste(sigma_setting, collapse = "-"),
                    ", Edge", paste(edge_setting, collapse = "-")),
      x = "Distance",
      y = "Correlation"
    ) +
    theme_minimal()
...
prior_ranges <- list(c(0.5, 0.5), c(1, 0.5), c(2, 0.5), c(0.5, 0.25), c(0.5, 0.75))
...
```{r}
prior_ranges <- list(c(0.5, 0.5))
prior_sigmas <- list(c(0.5, 0.5), c(0.25, 0.5), c(0.75, 0.5), c(0.5, 0.25), c(0.5, 0.75))
max_edges <- list(c(1, 2))
# Combination grid
settings <- expand.grid(
  prior.range = 1:length(prior_ranges),
...
prior_ranges <- list(c(0.5, 0.5))
prior_sigmas <- list(c(0.5, 0.5))
max_edges <- list(c(1, 2), c(1, 3), c(2, 3))
# Combination grid
settings <- expand.grid(
  prior.range = 1:length(prior_ranges),
  prior.sigma = 1:length(prior_sigmas),
...
**Explanation**: For the prior.range parameter in the SPDE model, increasing either of the parameters broadens the distribution of the correlation range. Specifically, the second parameter affects the correlation at larger distances more significantly, leading to wider confidence intervals that reflect increased uncertainty.

Similarly, for the prior.sigma parameter, increasing the values broadens the distribution of the standard deviation and increases uncertainty in variance estimates.

Regarding the spatial mesh resolution, increasing the max.edge parameter results in a coarser mesh. This less granular setup enhances the correlations between more distant locations because fewer distinctions between closely spaced observations are made.
...
```{r}
plot(year.mesh$loc,res.ozone1$summary.fixed$mean[1]*year.mesh$loc+res.ozone1$summary.random$fyear$mean,xlab="year",ylab="mean of total effect (fixed + random effect)",type="l")
```
...
**Explanation**: Regarding the yearly effects, there is an observable increasing trend over the years with greater volatility in the earlier years. The map indicates that urban areas tend to have lower ozone levels compared to surrounding regions, with ozone levels increasing in areas closer to the ocean.
